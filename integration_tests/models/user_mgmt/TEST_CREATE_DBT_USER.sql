...
use_case_01 as (
    {% set (initial_dbt_executor_pw, dbt_executor_user_name, dbt_executor_role_name, default_wh_name, default_db_name)
          =('Ch4ng3.ME!', 'PRJ_DBT_EXECUTOR', 'DBT_EXECUTOR_ROLE', 'PROJECT_DEV_WH', 'PROJECT_DEV' ) %}
    SELECT 
    ...
    '{{ sf_project_admin.create_user(
        dbt_executor_user_name, dbt_executor_role_name, initial_dbt_executor_pw, default_wh_name, default_db_name) |e }}' as result,

    'USE ROLE TEST_USERADMIN;' as validate_user_admin,      -- from default setting for project
    '{{'CREATE USER IF NOT EXISTS "PRJ_DBT_EXECUTOR"'|e }}' as validate_user_name,
    '{{"PASSWORD = 'Ch4ng3.ME!'" | e }}' as validate_pw,
