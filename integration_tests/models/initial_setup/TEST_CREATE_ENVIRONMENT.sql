WITH
use_case_01 as (
    {% set (prj_name,  env_name, owner_role, creator_role, useradmin_role, future_grants_role, single_WH)
          =('SAMPLE', 'XDEV', 'SOME_OWNER', 'SOME_CREATOR', 'SOME_USERADMIN', 'SECURITYADMIN', true) %}
    SELECT 
    ...
    '{{ sf_project_admin.create_environment(prj_name, env_name, owner_role, creator_role, useradmin_role, future_grants_role, single_WH) |e }}' as result,
    ...
    'GRANT SELECT ON FUTURE TABLES IN DATABASE SAMPLE_XDEV TO ROLE SAMPLE_XDEV_RO;' as validate_future_grants
)
, use_case_02 as (
    {% set (prj_name,  env_name, owner_role, creator_role, useradmin_role, future_grants_role, single_WH)
          =('SAMPLE', 'XDEV', 'SOME_OWNER', 'SOME_CREATOR', 'SOME_USERADMIN', none, true) %}
    SELECT 
    '{{prj_name}}' as prj_name, 
    '{{env_name}}' as env_name, 
    '{{owner_role}}' as owner_role, 
